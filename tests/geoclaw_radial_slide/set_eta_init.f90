...
subroutine set_eta_init(mbc,mx,my,xlow,ylow,dx,dy,t,veta)
    ...
    real(kind=8) :: x,y,r2,r,r3,eta2,eta3,eta_slope
    ...
    do j=1,my
        ...
        do i=1,mx
            x = xlow + (i-0.5d0)*dx
            r = sqrt(x**2 + y**2)
            !veta(i,j) = 200.d0 * min(1.d0, (1.d0 - (r - 2.d3)/10.d0))
            if (r > 1100.d0 .and. r < r3) then
                if (r < r2) then
                    veta(i,j) = eta2
                else
                    veta(i,j) = eta2 + (r-r2)*eta_slope
                endif
            endif

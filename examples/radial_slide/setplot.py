...
def setplot(plotdata=None):
    ...
    def aa(current_data):
        from pylab import cos, gca, pi, savefig, ticklabel_format, xticks
    ...
    def pure_water(current_data):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            m = hm / h
        water = np.where(np.logical_and(h > 1e-3, m < 0.1), eta, np.nan)
    ...
    plotitem.pcolor_cmin = sea_level - 20.0
    plotitem.pcolor_cmax = sea_level + 20.0
    plotitem.add_colorbar = True
    ...
    def landslide(current_data):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            m = hm / h
        landslide = np.where(np.logical_and(h > 1e-3, m > 0.1), h, np.nan)
        # import pdb; pdb.set_trace()
        return landslide
    ...
    cmap_mass = colormaps.make_colormap({0.0: "w", 1.0: "brown"})
    ...
    def plot_xsec(current_data):
        ...
        from pylab import (arange, fill_between, grid, legend, linspace,
                           loadtxt, logical_and, nan, ones, plot, sqrt, where,
                           xlabel, xlim, ylim, zeros)
        ...
        # plot(xout, etaout, 'm')
        fill_between(rout, Bout, water, color=[0.4, 0.4, 1])
    ...
    def s_vs_r(current_data):
        # Return radius of each grid cell and p value in the cell
        from pylab import nan, sqrt, where

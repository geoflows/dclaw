...
contains
        ...
        if (.not.module_setup) then
            ...
            do i=1,FGOUT_num_grids
               ...
               if (fg%end_time <= fg%start_time) then
                ...
                ! for geoclaw num_vars is 6
                ! h, hu, hv, bathy, eta, time
                ! so that interpolation can be done in time and space

                ! for dclaw num_vars is 10
                ! h, hu, hv, hm, pb, hchi, b_eroded, bathy, eta, time
    ...
    subroutine fgout_write(fgrid,out_time,out_index)
        ...
        ! Other locals
        integer :: i,j,m
        real(kind=8) :: t0,tf,tau, qaug(10) ! DIG switch from 6 to 10
        real(kind=8), allocatable :: qeta(:,:,:)
        real(kind=4), allocatable :: qeta4(:,:,:)
        real(kind=8) :: h_early,h_late,topo_early,topo_late

        allocate(qeta(8, fgrid%mx, fgrid%my))  ! to store h,hu,hv,hm,pb,hchi,b_eroded,eta # DIG
        ...
        do j=1,fgrid%my
            do i=1,fgrid%mx
                ...
                if (method == 0) then
                    ...
                    if (qaug(1) > 0.d0) then
                        topo_early = fgrid%early(8,i,j) ! DIG num_var - 2 (was 4 is now 8)
                        topo_late = fgrid%late(8,i,j) ! DIG num_var - 2 (was 4 is now 8)
                ...
                ! DIG change assignments.
                qeta(1,i,j) = qaug(1)  ! h
                qeta(2,i,j) = qaug(2)  ! hu
                qeta(3,i,j) = qaug(3)  ! hv
                qeta(4,i,j) = qaug(4)  ! hm
                qeta(5,i,j) = qaug(5)  ! pb
                qeta(6,i,j) = qaug(6)  ! hchi
                qeta(7,i,j) = qaug(7)  ! b_eroded
                qeta(8,i,j) = qaug(9)  ! eta
                ! bathy is qaug(8), time is qaug(10)
            ...
            do j=1,fgrid%my
                do i=1,fgrid%mx
                    write(unit, "(50e26.16)") qeta(1,i,j),qeta(2,i,j), &
                                ...
                                qeta(5,i,j),qeta(6,i,j), &
                                qeta(7,i,j),qeta(8,i,j) ! DIG add additional elements of qeta
                enddo
        ...
        if (fgrid%output_format == 3) then
            ...
            open(unit=unit, file=fg_filename, status="unknown",    &
                 access='stream')
            allocate(qeta4(8, fgrid%mx, fgrid%my))  ! for 4-byte binary output ! DIG fix to size 8*mx*my
        ...
        write(unit, t_file_format) out_time, 8, 1, 0, 2, 0, file_format ! DIG changed num_eqn+1
        close(unit)

        ! DIG. might want to eventually write out aux for entrainment?
        ! This will *break* for entrainment in that our B is aux(1)-q(7)
        ! Fixed grids not yet supported for entrainment.

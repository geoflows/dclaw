...
!! clawpack/geoclaw/src/2d/shallow/fgmax_frompatch.f90 
!!
!! this version has a minor change (line 260 and below)
!! that uses speed for arrival time check if FG_NUM_VAL>1
 ...
 603        format('+++ ij: ',8i7)
        ...
        do indexk=1,fg_klist_length
            ...
            if (FG_NUM_VAL>1) then
                s = fg_values(2,indexk) !DLG: dclaw- is this correct? or should it be indexk? is fg_values(2,) speed?
            else 
                s = abs(eta-sea_level)
            endif

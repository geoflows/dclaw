...
c # KRB 6/20/2025 - commented out because these lines remove i_hf and i_hs
cc     # Copied here from b4step2 since need to do before saving to qc1d:
c      forall(i=1:mitot, j=1:mjtot, q(1,i,j)+q(i_hf,i,j) < dry_tolerance)
c        q(1,i,j) = max(q(1,i,j),0.d0)
c        q(2:meqn,i,j) = 0.d0
c      end forall
cc
